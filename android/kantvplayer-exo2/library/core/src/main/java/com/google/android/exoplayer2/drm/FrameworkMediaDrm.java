...
public final class FrameworkMediaDrm implements ExoMediaDrm {
  ...
  public static final Provider DEFAULT_PROVIDER =
      uuid -> {
        try {
          ...
          KANTVLog.j(TAG, "Failed to instantiate a FrameworkMediaDrm for uuid: " + uuid + ".");
  ...
  public KeyRequest getKeyRequest(
    ...
    try {
      initDataString = new String(initData, "utf-8");
      KANTVLog.j(TAG, "initData:(" + initData.length + ") " + initDataString);
    } catch (Exception e) {
      KANTVLog.j(TAG, "error:" + e.toString());
    }

    MediaDrm.KeyRequest request = null;
    //2022-1-29,ZhouWeiguo, add for WisePlay DRM
    String wiseplayPSSHPrefix = "URI=\"data:text/plain;base64,";
    int wiseplayPSSHIndex = initDataString.indexOf(wiseplayPSSHPrefix);
    KANTVLog.d(TAG, "wiseplayPSSHIndex:" + wiseplayPSSHIndex);
    if (KANTVUtils.getEnableWisePlay() && wiseplayPSSHIndex > 0) {
      try {
        String wiseplayPSSH = initDataString.substring(wiseplayPSSHIndex + wiseplayPSSHPrefix.length(), initDataString.length() - 1);
        String wiseplayPSSHBase64Decode = "unknown pssh";
        KANTVDataEntity dataEntity = new KANTVDataEntity();
        if ((wiseplayPSSH != null) && (wiseplayPSSH.length() > 0)) {
          //not works
          //wiseplayPSSHBase64Decode = KANTVUtils.base64Decode(wiseplayPSSH);
          //using JNI
          int result = KANTVDRMManager.base64Decode(wiseplayPSSH.getBytes(), wiseplayPSSH.getBytes().length, dataEntity);
          if (dataEntity != null && dataEntity.getData() != null) {
            KANTVLog.j(TAG, "base64 decode:" + dataEntity.getResult() + ", content:" + new String(dataEntity.getData()));
          }

        }
        KANTVLog.j(TAG, "wiseplay PSSH:" + wiseplayPSSH);
        KANTVLog.j(TAG, "base64 decode wiseplay PSSH:" + KANTVUtils.bytesToHexString(dataEntity.getData()));
        request = mediaDrm.getKeyRequest(scope, dataEntity.getData(), mimeType, keyType, optionalParameters);
      } catch (Exception e) {
        KANTVLog.j(TAG, "base64Decode error:" + e.toString());
      }
      //2022-1-29,ZhouWeiguo, end add for WisePlay DRM
    } else {
      request = mediaDrm.getKeyRequest(scope, initData, mimeType, keyType, optionalParameters);
    }
