...
import kantvai.media.encoder.Crypto;
import kantvai.media.encoder.Util;
...
//        out.write(new byte[]{(byte) 0x80, 0x00, 0x07, 0x02}); // Bytes 4 - 7: Flash player version: 11.2.202.233
        ...
        KANTVLog.j(TAG, "writeC1(): Calculating digest offset");
        ...
        KANTVLog.j(TAG, "writeC1(): (real value of) digestOffset: " + digestOffset);


        KANTVLog.j(TAG, "writeC1(): recalculated digestOffset: " + absoluteDigestOffset);
        ...
        KANTVLog.j(TAG, "writeC1(): digestOffsetBytes: " + Util.toHexString(digestOffsetBytes));  //Util.unsignedInt32ToByteArray((digestOffset % 728))));

        // Create random bytes up to the digest offset point
        byte[] partBeforeDigest = new byte[absoluteDigestOffset];
        KANTVLog.j(TAG, "partBeforeDigest(): size: " + partBeforeDigest.length);
        random.nextBytes(partBeforeDigest);

        KANTVLog.j(TAG, "writeC1(): Writing timestamp and Flash Player version");
        ...
        KANTVLog.j(TAG, "partAfterDigest(): size: " + partAfterDigest.length);
        random.nextBytes(partAfterDigest);


        // Set the offset byte
//        if (digestOffset > 772) {                      
        KANTVLog.j(TAG, "copying digest offset bytes in partBeforeDigest");
...
//        }
        ...
        KANTVLog.j(TAG, "writeC1(): Calculating digest");
        ...
        KANTVLog.j(TAG, "writeC1(): writing C1 packet");
    ...
    public final void readS2(InputStream in) throws IOException {
        ...
        if (totalBytesRead != remainingBytes) {
            ...
            KANTVLog.j(TAG, "readS2(): S2 total bytes read OK");
        }
