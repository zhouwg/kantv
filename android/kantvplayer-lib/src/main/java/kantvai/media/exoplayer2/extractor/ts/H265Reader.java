...
public final class H265Reader implements ElementaryStreamReader {
  private static final String TAG = H265Reader.class.getName();
  ...
  public H265Reader(SeiReader seiReader) {
    ...
    if ((KANTVUtils.getDecryptMode() == KANTVUtils.DECRYPT_SOFT) || KANTVUtils.getEnableMultiDRM()) {
      dataHolder = new DataHolder(1024 * 1024); //FIXME
    }
  }
  ...
  public void packetFinished() {
    ...
    if ((KANTVUtils.getDecryptMode() == KANTVUtils.DECRYPT_SOFT) || KANTVUtils.getEnableMultiDRM()) {
      ...
      ParsableByteArray naluData = new ParsableByteArray(dataEntity.getData(), dataEntity.getDataLength());
  ...
  private void consumeCachedData(ParsableByteArray data) {
    //KANTVLog.d(TAG, "Play the decrypted frames, bytesLeft length:" + data.bytesLeft());
    while (data.bytesLeft() > 0) {
      int offset = data.getPosition();
      int limit = data.limit();
      byte[] dataArray = data.getData();
