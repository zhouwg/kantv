...
import static kantvai.media.exoplayer2.extractor.ts.TsPayloadReader.FLAG_PAYLOAD_UNIT_START_INDICATOR;
...
import kantvai.media.exoplayer2.extractor.ts.DefaultTsPayloadReaderFactory.Flags;
import kantvai.media.exoplayer2.extractor.ts.TsPayloadReader.DvbSubtitleInfo;
import kantvai.media.exoplayer2.extractor.ts.TsPayloadReader.EsInfo;
...
import kantvai.media.exoplayer2.util.ParsableBitArray;
...
public final class TsExtractor implements Extractor {
  private static final String TAG = TsExtractor.class.getName();
  ...
  public static final int TS_STREAM_TYPE_SAMPLE_AES_H264 = 0xDB;//weiguo, this is sample-aes
  public static final int TS_STREAM_TYPE_SAMPLE_AES_H265 = 0xE4;

  public static final int TS_STREAM_TYPE_DOLBY_VISION = 0x06; //weiguo, Dolby Vision HLS encrypted content
  ...
  private static final long DOLBY_VISION_FORMAT_IDENTIFIER = 0x444f5649;//weiguo, support Dolby Vision HLS encrypted content
  ...
  public @ReadResult int read(ExtractorInput input, PositionHolder seekPosition)
    ...
    */
    ...
    //mode is MODE_HLS at the moment
    // Discontinuity check.
    if (mode != MODE_HLS) {
  ...
  private int findEndOfFirstTsPacketInBuffer() throws ParserException {
    ...
    if (endOfPacket > limit) {
      ...
      if (mode == MODE_HLS && bytesSinceLastSync > TS_PACKET_SIZE * 2) {
        KANTVLog.j(TAG, "Cannot find sync byte. Most likely not a Transport Stream");
  ...
  private class PmtReader implements SectionPayloadReader {
    ...
    public void consume(ParsableByteArray sectionData) {
      ...
      if (0 == KANTVUtils.isClearContent())
      {
        int result = KANTVDRMManager.setDrmInfo(sectionData.getData());
        if (result != 0 && result != -2) {
          KANTVLog.j(TAG, "pls check why failed to set drm info:" + result);
          //System.exit(0);
          //return;
        }
      }
    ...
    private EsInfo readEsInfo(ParsableByteArray data, int length) {
      ...
      while (data.getPosition() < descriptorsEndPosition) {
        ...
        if (descriptorTag == TS_PMT_DESC_REGISTRATION) { // registration_descriptor
          ...
          } else if (formatIdentifier == DOLBY_VISION_FORMAT_IDENTIFIER) {
            KANTVLog.j(TAG, "found dolby vision content");
            streamType = TS_STREAM_TYPE_DOLBY_VISION;
          } else {
            KANTVLog.j(TAG, "unknown stream type: " + Integer.toHexString((int) formatIdentifier));
          }
          ...
          KANTVLog.j(TAG, "found dolby eac3 content");
          streamType = TS_STREAM_TYPE_E_AC3;
