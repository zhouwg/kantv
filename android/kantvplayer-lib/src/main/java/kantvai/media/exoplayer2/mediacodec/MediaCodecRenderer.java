 ...
 */
package kantvai.media.exoplayer2.mediacodec;

import static kantvai.media.exoplayer2.decoder.DecoderReuseEvaluation.DISCARD_REASON_DRM_SESSION_CHANGED;
import static kantvai.media.exoplayer2.decoder.DecoderReuseEvaluation.DISCARD_REASON_OPERATING_RATE_CHANGED;
import static kantvai.media.exoplayer2.decoder.DecoderReuseEvaluation.DISCARD_REASON_REUSE_NOT_IMPLEMENTED;
import static kantvai.media.exoplayer2.decoder.DecoderReuseEvaluation.DISCARD_REASON_WORKAROUND;
import static kantvai.media.exoplayer2.decoder.DecoderReuseEvaluation.REUSE_RESULT_NO;
import static kantvai.media.exoplayer2.decoder.DecoderReuseEvaluation.REUSE_RESULT_YES_WITHOUT_RECONFIGURATION;
import static kantvai.media.exoplayer2.decoder.DecoderReuseEvaluation.REUSE_RESULT_YES_WITH_FLUSH;
import static kantvai.media.exoplayer2.decoder.DecoderReuseEvaluation.REUSE_RESULT_YES_WITH_RECONFIGURATION;
import static kantvai.media.exoplayer2.source.SampleStream.FLAG_OMIT_SAMPLE_DATA;
import static kantvai.media.exoplayer2.source.SampleStream.FLAG_PEEK;
...
import kantvai.media.exoplayer2.decoder.CryptoInfo;
import kantvai.media.exoplayer2.decoder.DecoderCounters;
import kantvai.media.exoplayer2.decoder.DecoderInputBuffer;
import kantvai.media.exoplayer2.decoder.DecoderInputBuffer.InsufficientCapacityException;
import kantvai.media.exoplayer2.decoder.DecoderReuseEvaluation;
import kantvai.media.exoplayer2.decoder.DecoderReuseEvaluation.DecoderDiscardReasons;
...
import kantvai.media.exoplayer2.drm.FrameworkMediaCrypto;
import kantvai.media.exoplayer2.mediacodec.MediaCodecUtil.DecoderQueryException;
import kantvai.media.exoplayer2.source.MediaPeriod;
import kantvai.media.exoplayer2.source.SampleStream;
import kantvai.media.exoplayer2.source.SampleStream.ReadDataResult;
import kantvai.media.exoplayer2.source.SampleStream.ReadFlags;
...
import kantvai.media.exoplayer2.util.TimedValueQueue;
...
import kantvai.media.player.KANTVCryptoInfo;
...
public abstract class MediaCodecRenderer extends BaseRenderer {
  private static final String TAG = MediaCodecRenderer.class.getName();
  ...
  public static class DecoderInitializationException extends Exception {
    ...
    private static String buildCustomDiagnosticInfo(int errorCode) {
      ...
      return "kantvai.media.exoplayer2.mediacodec.MediaCodecRenderer_"
          + sign
          + Math.abs(errorCode);
    }
  }
  ...
  protected final void maybeInitCodecOrBypass() throws ExoPlaybackException {
    ...
    KANTVLog.j(TAG, "mimeType:" + mimeType);
    if (codecDrmSession != null) {
      if (mediaCrypto == null) {
        ...
        if (sessionMediaCrypto == null) {
          ...
          try {
            //weiguo
            KANTVLog.j(TAG, "create MediaCrypto instance for " + sessionMediaCrypto.uuid.toString() + "(" + KANTVUtils.getDrmName(sessionMediaCrypto.uuid) + ")");
          ...
          //weiguo
          if (C.CHINADRM_UUID.equals(sessionMediaCrypto.uuid) || C.WISEPLAY_UUID.equals(sessionMediaCrypto.uuid)) {
          ...
          KANTVLog.j(TAG, "requires secure decoder: " + mediaCryptoRequiresSecureDecoder);
        }
      }
    ...
    try {
      //weiguo
      if ((null != codecDrmSession) && (C.CHINADRM_UUID.equals(codecDrmSession.getSchemeUuid()))) {
        if (!mimeType.startsWith(MimeTypes.BASE_TYPE_VIDEO)) {
      ...
      KANTVLog.j(TAG, "DecoderInitializationException:" + e.toString());
      throw createRendererException(
  ...
  private void maybeInitCodecWithFallback(
    ...
    while (codec == null) {
      ...
      try {
        ...
        KANTVLog.j(TAG, "Failed to initialize decoder: " + codecInfo + e.toString());
  ...
  private void initCodec(MediaCodecInfo codecInfo, MediaCrypto crypto) throws Exception {
    ...
    KANTVLog.j(TAG,"createCodec:" + codecName);
  ...
  private boolean feedInputBuffer() throws ExoPlaybackException {
    ...
    try {
      //weiguo: make wiseplay happy
      if (KANTVUtils.getEnableWisePlay()) {
        boolean enableDebug = true;
        int len = buffer.data.limit() - buffer.data.position();
        byte[] bytes = new byte[len];
        KANTVCryptoInfo dataEntity = KANTVCryptoInfo.getInstance();
        buffer.data.get(bytes, 0, bytes.length);
        KANTVDRMManager.parseData(bytes, len, dataEntity);
      }

      if (bufferEncrypted) {
        KANTVLog.j(TAG, "queueSecureInputBuffer");
