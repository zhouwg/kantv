...
package com.kantvai.kantvplayer.player.ffplayer;
...
import com.kantvai.kantvplayer.player.common.utils.TrackInfoUtils;
import com.kantvai.kantvplayer.player.common.widgets.BottomBarView;

import com.kantvai.kantvplayer.player.common.widgets.DialogScreenShot;

import com.kantvai.kantvplayer.player.common.widgets.SettingPlayerView;
import com.kantvai.kantvplayer.player.common.widgets.SettingSubtitleView;
import com.kantvai.kantvplayer.player.common.widgets.SkipTipView;
import com.kantvai.kantvplayer.player.common.widgets.TopBarView;

import com.kantvai.kantvplayer.player.ffplayer.media.KANTVVideoView;
import com.kantvai.kantvplayer.player.ffplayer.media.MediaPlayerParams;
import com.kantvai.kantvplayer.player.subtitle.SubtitleManager;
import com.kantvai.kantvplayer.player.subtitle.SubtitleParser;
import com.kantvai.kantvplayer.player.subtitle.SubtitleView;
...
public class FFPlayerView extends FrameLayout implements PlayerViewListener {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case MSG_UPDATE_SUBTITLE:
                    KANTVLog.j(TAG, "update subtitle");
                ...
                case MSG_SET_SUBTITLE_SOURCE:
                    KANTVLog.j(TAG, "update subtitle source");
    ...
    private void initViewBefore(Context context) {
        ...
        mOrientationListener = new OrientationEventListener(mAttachActivity) {
            ...
            public void onOrientationChanged(int orientation) {
                KANTVLog.d(TAG, "on orientation changed, orientation is:" + orientation);
                if (mIsNeverPlay) {
                    return;
                }
    ...
    private void initViewCallBak() {
        ...
        IMediaPlayer.OnPreparedListener ijkPreparedCallback = iMediaPlayer -> {
            KANTVLog.d(TAG, "on prepared listener");
        ...
        IMediaPlayer.OnErrorListener ijkErrorCallback = (iMediaPlayer, i, i1) -> {
            KANTVLog.d(TAG, "on error listener");
        ...
        IMediaPlayer.OnInfoListener ijkPlayInfoCallback = (iMediaPlayer, status, extra) -> {
            KANTVLog.d(TAG, "on info listener");
            _switchStatus(status);
            return true;
        };

        IMediaPlayer.OnTimedTextListener ijkPlayTimedTextListener = (mp, text) -> {
            KANTVLog.d(TAG, "recv subtitle: " + text.getText());
            if (!KANTVUtils.getTVASR())
                return;
        ...
        IMediaPlayer.OnTrackChangeListener ijkPlayTrackChangeListener = (mp, trackSelector, trackGroups, trackSelections) -> {
            KANTVLog.d(TAG, "on track changed listener");
            /*
            if ((mp != null) && (mp instanceof CDEOSExo2MediaPlayer)) {
                KANTVLog.j(TAG, "track change");
                TrackInfoUtils trackInfoUtils = new TrackInfoUtils();
        ...
        TopBarView.TopBarListener topBarCallBack = new TopBarView.TopBarListener() {
            @Override
            public void onBack() {
                KANTVLog.d(TAG, "onBack because back button on top view");
    ...
    public void initSubtitleSettingView() {
        ...
        if (mSettings.getPlayerEngine() == KANTVUtils.PV_PLAYERENGINE__AndroidMediaPlayer) {
            KANTVLog.j(TAG, "subtitle not supported with AndroidMediaPlayer");
        }
        ...
        topBarView.getPlayerSettingView()
                .setSettingListener(new SettingPlayerView.SettingVideoListener() {
                    ...
                    public void setOrientationStatus(boolean isEnable) {
                        KANTVLog.j(TAG, "orientation status :" + isEnable);
                    ...
                    public void setLandscapeStatus(boolean bLandScape) {
                        KANTVLog.j(TAG, "landscape :" + bLandScape);
                    ...
                    public void setTVRecordingStatus(boolean bRecording) {
                        int recordCountsFromServer = KANTVUtils.getRecordCountsFromServer();
                        boolean isPaidUser = KANTVUtils.getIsPaidUser();
                        KANTVLog.j(TAG, "tv recording :" + bRecording);
                        KANTVLog.j(TAG, "current video name:" + mVideoName);
                        KANTVLog.j(TAG, "current video url:" + mVideoUrl);
                        KANTVLog.j(TAG, "recordCountsFromServer:" + recordCountsFromServer);
                        KANTVLog.j(TAG, "isPaidUser:" + isPaidUser);
...

                        {
                            ...
                            if (diskFreeSize < KANTVUtils.getDiskThresholdFreeSize()) {
                    ...
                    public void setTVASRStatus(boolean bEnableASR) {
                        KANTVLog.j(TAG, "enable ASR:" + bEnableASR);
                        topBarView.hideItemView();
    ...
    public void onPause() {
        ...
        if ((mVideoView != null) && (mVideoView.isBackgroundPlayEnabled())) {
            ...
            KANTVLog.j(TAG, "detect pause, exit playback");
            onBackPressed();
        }
    }
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        ...
        KANTVLog.j(TAG, "keyCode:" + keyCode);
        if (keyCode == KeyEvent.KEYCODE_VOLUME_UP) {
            _setVolume(true);
            return true;
        } else if (keyCode == KeyEvent.KEYCODE_VOLUME_DOWN) {
    ...
    public boolean onBackPressed() {
        ...
        KANTVLog.j(TAG, "isBackgroundEnabled:" + mVideoView.isBackgroundPlayEnabled());
    ...
    public FFPlayerView setVideoPath(String videoPath) {
        ...
        KANTVLog.j(TAG, "video url:" + videoPath);
        seekTo(0);
        return this;
    }
    ...
    public FFPlayerView setTitle(String title) {
        ...
        KANTVLog.j(TAG, "video name:" + title);
        return this;
    }
    ...
    public void start() {
        ...
        KANTVLog.d(TAG, "manually start play position:" + KANTVUtils.getStartPlayPos() + " minutes");
        KANTVLog.d(TAG, "last play position: " + mPlayPos + " secs");
        if (KANTVUtils.getStartPlayPos() != 0) {
            mPlayPos = KANTVUtils.getStartPlayPos() * 60;
        }
        if (mPlayPos != 0) {
    ...
    private void onRecordStart() {
        KANTVLog.j(TAG, "recording start");
        if ((KANTVUtils.PV_PLAYERENGINE__FFmpeg != mSettings.getPlayerEngine()) || (mSettings.getUsingMediaCodec())
        ) {
            Toast.makeText(getContext(), "pls check playback setting", Toast.LENGTH_SHORT).show();
            topBarView.updateTVRecordingVisibility(false);
        } else {
            KANTVUtils.setTVRecording(true);
            KANTVUtils.perfGetRecordBeginTime();
            topBarView.updateTVRecordingVisibility(true);
            KANTVLog.j(TAG, "recording filename:" + KANTVUtils.getRecordingFileName());
            ...
            if (0 == recordMode) {
                tipInfo = tipInfo + "video resolution:" + videoWidth + "x" + videoHeight + "\n"
                        ...
                        + "record mode:" + KANTVUtils.getRecordModeString(recordMode)
                        + ",file format:" + KANTVUtils.getRecordFormatString(recordFormat)
                        + ",video codec:" + KANTVUtils.getVideoCodecString(recordCodec)
                        + ",audio codec:" + "aac"
            ...
            KANTVLog.j(TAG, "record mode:" + recordMode);
            ...
            KANTVUtils.umStartRecord(KANTVUtils.getRecordingFileName());
        }
    }

    private void onRecordStop() {
        KANTVLog.j(TAG, "recording stop");
        if (!KANTVUtils.getTVRecording()) {
            KANTVLog.j(TAG, "record not started, why called again?");
            return;
        }
        KANTVUtils.setTVRecording(false);
        topBarView.updateTVRecordingVisibility(false);
        KANTVLog.j(TAG, "recording file:" + KANTVUtils.getRecordingFileName());
        File file = new File(KANTVUtils.getRecordingFileName());
        if (file.exists()) {
            KANTVLog.j(TAG, "recording file " + KANTVUtils.getRecordingFileName() + ", size:" + file.length() + " bytes");
            Toast.makeText(getContext(), "recording stopped，size of recorded file: " + KANTVUtils.formattedSize(file.length()), Toast.LENGTH_LONG).show();
            KANTVUtils.umStopRecord(KANTVUtils.getRecordingFileName());
        } else {
            KANTVLog.j(TAG, "it shouldn't happen");
            Toast.makeText(getContext(), "recording stopped", Toast.LENGTH_SHORT).show();
            KANTVUtils.umStopRecord("unknown");
        }
    ...
    private void onASRStart(int asrMode) {
        ...
        if (!file.exists()) {
            ...
            topBarView.updateTVASRVisibility(false);
            KANTVUtils.setTVASR(false);
            KANTVUtils.showMsgBox(mAttachActivity, "GGML model file not found:" + file.getAbsolutePath());
            return;
        } else {
        ...
        if (KANTVUtils.getASRSubsystemInit()) {
            if ((KANTVUtils.ASR_MODE_NORMAL == mSettings.getASRMode()) || (KANTVUtils.ASR_MODE_TRANSCRIPTION_RECORD == mSettings.getASRMode())) {
                ggmljava.asr_reset(KANTVUtils.getDataPath() + "/models/" + ggmlModelFileName, mSettings.getASRThreadCounts(), KANTVUtils.ASR_MODE_NORMAL, KANTVUtils.HEXAGON_BACKEND_GGML);
            } else {
                ggmljava.asr_reset(KANTVUtils.getDataPath() + "/models/" + ggmlModelFileName, mSettings.getASRThreadCounts(), KANTVUtils.ASR_MODE_PRESURETEST, KANTVUtils.HEXAGON_BACKEND_GGML);
            }
            ggmljava.asr_start();
        } else {
            topBarView.updateTVASRVisibility(false);
    ...
    private void onASRStop(int asrMode) {
        KANTVLog.j(TAG, "ASR stop");
        if (!KANTVUtils.getTVASR()) {
            KANTVLog.j(TAG, "TV transcription not started, why called?");
            return;
        }
        ...
        if (asrMode == KANTVUtils.ASR_MODE_TRANSCRIPTION_RECORD) {
            ...
            if (file.exists()) {
                KANTVLog.j(TAG, "saved raw audio data file " + KANTVUtils.getASRSavedFileName() + ", size:" + file.length() + " bytes");
                Toast.makeText(getContext(), "TV transcription stopped，size of saved raw audio data file: "
                        + KANTVUtils.formattedSize(file.length()), Toast.LENGTH_LONG).show();
            } else {
                KANTVLog.j(TAG, "it shouldn't happen");
            ...
            KANTVLog.j(TAG, "TV transcription stopped");
            Toast.makeText(getContext(), "TV transcription stopped", Toast.LENGTH_SHORT).show();
        }
    ...
    private void _switchStatus(int status) {
        KANTVLog.d(TAG, "status:" + status);
        switch (status) {
            case 5:
                KANTVLog.j(TAG, "receive playback completed");
                stopRecord();
                stopASR();
                break;
            case 1983:
                ...
                KANTVLog.j(TAG, "recording stop");
            {
                File file = new File(KANTVUtils.getRecordingFileName());
                if (file.exists()) {
                    KANTVLog.j(TAG, "recording file size:" + file.length());
                    //Toast.makeText(getContext(), "recording finished，size of recorded file:" + KANTVUtils.formattedSize(file.length()), Toast.LENGTH_LONG).show();
                } else {
    ...
    private void hideShowBar(boolean isShow) {
        ...
        if (mVideoView.isExoPlayer()) {
            topBarView.updateTVRecordingVisibility(KANTVUtils.getTVRecording());
        }
    }
