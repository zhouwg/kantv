...
public final class FrameworkMediaDrm implements ExoMediaDrm {
  ...
  public static final Provider DEFAULT_PROVIDER =
      uuid -> {
        try {
          ...
          CDELog.j(TAG, "Failed to instantiate a FrameworkMediaDrm for uuid: " + uuid + ".");
  ...
  public KeyRequest getKeyRequest(
    ...
    CDELog.j(TAG, "mimeType:" + mimeType);

    try {
      initDataString = new String(initData, "utf-8");
      CDELog.j(TAG, "initData:(" + initData.length + ") " + initDataString);
    } catch (Exception e) {
    ...
    CDELog.d(TAG, "wiseplayPSSHIndex:" + wiseplayPSSHIndex);
    if (CDEUtils.getEnableWisePlay() && wiseplayPSSHIndex > 0) {
      try {
        ...
        if ((wiseplayPSSH != null) && (wiseplayPSSH.length() > 0)) {
          //not works
          //wiseplayPSSHBase64Decode = CDEUtils.base64Decode(wiseplayPSSH);
          //using JNI
          ...
          if (dataEntity != null && dataEntity.getData() != null) {
            CDELog.j(TAG, "base64 decode:" + dataEntity.getResult() + ", content:" + new String(dataEntity.getData()));
          }

        }
        CDELog.j(TAG, "wiseplay PSSH:" + wiseplayPSSH);
        CDELog.j(TAG, "base64 decode wiseplay PSSH:" + CDEUtils.bytesToHexString(dataEntity.getData()));
