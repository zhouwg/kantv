...
import cdeos.media.player.CDECryptoInfo;
...
public abstract class MediaCodecRenderer extends BaseRenderer {
  ...
  protected final void maybeInitCodecOrBypass() throws ExoPlaybackException {
    ...
    if (codecDrmSession != null) {
      if (mediaCrypto == null) {
        ...
        if (sessionMediaCrypto == null) {
          ...
          try {
            //weiguo
            CDELog.j(TAG, "create MediaCrypto instance for " + sessionMediaCrypto.uuid.toString() + "(" + CDEUtils.getDrmName(sessionMediaCrypto.uuid) + ")");
          ...
          CDELog.j(TAG, "requires secure decoder: " + mediaCryptoRequiresSecureDecoder);
        }
      }
    ...
    try {
      ...
      CDELog.j(TAG, "DecoderInitializationException:" + e.toString());
      throw createRendererException(
  ...
  private void maybeInitCodecWithFallback(
    ...
    while (codec == null) {
      ...
      try {
        ...
        CDELog.j(TAG, "Failed to initialize decoder: " + codecInfo + e.toString());
  ...
  private void initCodec(MediaCodecInfo codecInfo, MediaCrypto crypto) throws Exception {
    ...
    CDELog.j(TAG,"createCodec:" + codecName);
  ...
  private boolean feedInputBuffer() throws ExoPlaybackException {
    ...
    try {
      ...
      if (CDEUtils.getEnableWisePlay()) {
        ...
        CDECryptoInfo dataEntity = CDECryptoInfo.getInstance();
        buffer.data.get(bytes, 0, bytes.length);
        KANTVDRMManager.parseData(bytes, len, dataEntity);
      }

      if (bufferEncrypted) {
        CDELog.j(TAG, "queueSecureInputBuffer");
