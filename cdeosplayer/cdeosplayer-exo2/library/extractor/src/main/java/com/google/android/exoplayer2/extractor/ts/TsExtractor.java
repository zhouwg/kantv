...
public final class TsExtractor implements Extractor {
  ...
  private int findEndOfFirstTsPacketInBuffer() throws ParserException {
    ...
    if (endOfPacket > limit) {
      ...
      if (mode == MODE_HLS && bytesSinceLastSync > TS_PACKET_SIZE * 2) {
        CDELog.j(TAG, "Cannot find sync byte. Most likely not a Transport Stream");
  ...
  private class PmtReader implements SectionPayloadReader {
    ...
    public void consume(ParsableByteArray sectionData) {
      ...
      if (0 == CDEUtils.isClearContent())
      {
        int result = KANTVDRMManager.setDrmInfo(sectionData.getData());
        if (result != 0 && result != -2) {
          CDELog.j(TAG, "pls check why failed to set drm info:" + result);
          //System.exit(0);
          //return;
        }
      }
      ...
      while (data.getPosition() < descriptorsEndPosition) {
        ...
        if (descriptorTag == TS_PMT_DESC_REGISTRATION) { // registration_descriptor
          ...
          if (formatIdentifier == AC3_FORMAT_IDENTIFIER) {
            ...
            CDELog.j(TAG, "found dolby vision content");
            streamType = TS_STREAM_TYPE_DOLBY_VISION;
          } else {
            CDELog.j(TAG, "unknown stream type: " + Integer.toHexString((int) formatIdentifier));
          }
          ...
          CDELog.j(TAG, "found dolby eac3 content");
          streamType = TS_STREAM_TYPE_E_AC3;
