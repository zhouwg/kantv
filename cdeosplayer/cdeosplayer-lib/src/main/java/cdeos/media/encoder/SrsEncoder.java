...
import cdeos.media.encoder.SrsFlvMuxer;
...
public class SrsEncoder {
    ...
    public boolean start() {
        ...
        if (!useSoftEncoder && (vOutWidth % 32 != 0 || vOutHeight % 32 != 0)) {
            if (vmci.getName().contains("MTK")) {
                ...
                CDELog.j(TAG, "found MTK encoder");
            }
        }

        CDELog.j(TAG, "set encoder resolution");
        ...
        try {
            ...
            CDELog.j(TAG, "create audio encoder failed.");
            e.printStackTrace();
            return false;
        }
        ...
        try {
            ...
            CDELog.j(TAG, "create video encoder failed.");
            e.printStackTrace();
            return false;
        }
    ...
    private MediaCodecInfo chooseVideoEncoder(String name) {
        CDELog.j(TAG, "enter chooseVideoEncoder");
        CDELog.j(TAG, "calling MediaCodecList.getCodecCount()");
        MediaCodecList mediaCodecList = new MediaCodecList(MediaCodecList.ALL_CODECS);

        MediaCodecInfo[] codecInfos = mediaCodecList.getCodecInfos();
        CDELog.j(TAG, "codec counts: " + codecInfos.length);
        for (MediaCodecInfo mci : codecInfos) {
            CDELog.j(TAG, "codecInfo = " + mci.getName());

            if (!mci.isEncoder()) {
                CDELog.j(TAG, mci.getName() + " " + " is not encoder");
                continue;
            }
            ...
            for (int j = 0; j < types.length; j++) {
                if (types[j].equalsIgnoreCase(VCODEC)) {
                    CDELog.j(TAG, "here");
                    CDELog.j(TAG, String.format("vencoder %s types: %s", mci.getName(), types[j]));
                    if (name == null) {
                        CDELog.j(TAG, "found valid encoder: " + mci.getName());
                        return mci;
                    }
        ...
        CDELog.j(TAG, "leave chooseVideoEncoder");
        return null;
    }
    ...
    private int chooseVideoEncoder() {
        ...
        for (int i = 0; i < cc.colorFormats.length; i++) {
            ...
            CDELog.j(TAG, String.format("vencoder %s supports color fomart 0x%x(%d)", vmci.getName(), cf, cf));
        ...
        for (int i = 0; i < cc.profileLevels.length; i++) {
            ...
            CDELog.j(TAG, String.format("vencoder %s support profile %d, level %d", vmci.getName(), pl.profile, pl.level));
        }

        CDELog.j(TAG, String.format("vencoder %s choose color format 0x%x(%d)", vmci.getName(), matchedColorFormat, matchedColorFormat));
