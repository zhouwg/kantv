package cdeos.media.encoder.packets;
...
import cdeos.media.encoder.Crypto;
import cdeos.media.encoder.Util;
...
//        out.write(new byte[]{(byte) 0x80, 0x00, 0x07, 0x02}); // Bytes 4 - 7: Flash player version: 11.2.202.233
        ...
        CDELog.j(TAG, "writeC1(): Calculating digest offset");
        ...
        CDELog.j(TAG, "writeC1(): (real value of) digestOffset: " + digestOffset);


        CDELog.j(TAG, "writeC1(): recalculated digestOffset: " + absoluteDigestOffset);
        ...
        CDELog.j(TAG, "writeC1(): digestOffsetBytes: " + Util.toHexString(digestOffsetBytes));  //Util.unsignedInt32ToByteArray((digestOffset % 728))));

        // Create random bytes up to the digest offset point
        byte[] partBeforeDigest = new byte[absoluteDigestOffset];
        CDELog.j(TAG, "partBeforeDigest(): size: " + partBeforeDigest.length);
        random.nextBytes(partBeforeDigest);

        CDELog.j(TAG, "writeC1(): Writing timestamp and Flash Player version");
        ...
        CDELog.j(TAG, "partAfterDigest(): size: " + partAfterDigest.length);
        random.nextBytes(partAfterDigest);


        // Set the offset byte
//        if (digestOffset > 772) {                      
        CDELog.j(TAG, "copying digest offset bytes in partBeforeDigest");
...
//        }
        ...
        CDELog.j(TAG, "writeC1(): Calculating digest");
        ...
        CDELog.j(TAG, "writeC1(): writing C1 packet");
    ...
    public final void readS2(InputStream in) throws IOException {
        ...
        if (totalBytesRead != remainingBytes) {
            ...
            CDELog.j(TAG, "readS2(): S2 total bytes read OK");
        }
