    ...
    for (int j = 0; j < (int) sizes.size(); j++) {
        ...
        for (int k = 0; k < 7; ++k) {
            ...
            if (n_backend !=
                ...
                backend = ggml_backend_qnn_init(n_backend,
...
int llama_inference(const char *model_path, const char *prompt, int bench_type, int num_threads,
    ...
    if (n_backend !=
        3) { // 3 is the original GGML, used to compare performance between QNN backend and original GGML
    ...
    while ((n_remain != 0 && !is_antiprompt) || params.interactive) {
        ...
        // end of generation
        if (max_tokens >
            300) { //TODO: dirty method to fix issue:https://github.com/zhouwg/kantv/issues/116
            LOGGI(" [end of text]\n");
#ifdef TARGET_ANDROID
...
static std::string get_image_params(SDParams params, int64_t seed) {
    ...
    parameter_string +=
            "Size: " + std::to_string(params.width) + "x" + std::to_string(params.height) + ", ";
...
static int stablediffusion_main(int argc, const char *argv[]) {
    ...
    if (params.mode == IMG2IMG || params.mode == IMG2VID) {
        ...
        int c = 0;
        input_image_buffer = stbi_load(params.input_path.c_str(), &params.width, &params.height, &c,
                                       3);
    ...
    if (params.controlnet_path.size() > 0 && params.control_image_path.size() > 0) {
        int c = 0;
        control_image_buffer = stbi_load(params.control_image_path.c_str(), &params.width,
                                         &params.height, &c, 3);
    ...
    if (params.mode == TXT2IMG) {
        ...
        if (params.controlnet_path.size() > 0 && params.control_image_path.size() > 0) {
            int c = 0;
            input_image_buffer = stbi_load(params.control_image_path.c_str(), &params.width,
                                           &params.height, &c, 3);
            if (input_image_buffer == NULL) {
                LOGGD("load image from '%s' failed\n", params.control_image_path.c_str());
                return 1;
        ...
        if (params.mode == IMG2VID) {
            ...
            std::string dummy_name = last != std::string::npos ? params.output_path.substr(0, last)
                                                               : params.output_path;
            for (int i = 0; i < params.video_frames; i++) {
                ...
                std::string final_image_path =
                        i > 0 ? dummy_name + "_" + std::to_string(i + 1) + ".png" : dummy_name +
                                                                                    ".png";
                stbi_write_png(final_image_path.c_str(), results[i].width, results[i].height,
                               results[i].channel,
                               results[i].data, 0,
        ...
        if (upscaler_ctx == NULL) {
            ...
            for (int i = 0; i < params.batch_count; i++) {
                ...
                for (int u = 0; u < params.upscale_repeats; ++u) {
                    sd_image_t upscaled_image = upscale(upscaler_ctx, current_image,
                                                        upscale_factor);
    ...
    std::string dummy_name =
            last != std::string::npos ? params.output_path.substr(0, last) : params.output_path;
    for (int i = 0; i < params.batch_count; i++) {
        if (results[i].data == NULL) {
            continue;
        }
 ...
 * @return
*/
int stablediffusion_inference(const char *sz_model_path, const char *prompt, int bench_type,
...
int qnn_complex_graph(int n_backend_type, int n_graph_type) {
    ...
    switch (n_graph_type) {
        case 0: // MNIST CV inference of digital number, https://github.com/StudyingLover/ggml-tutorial
        {
            ...
            mnist_inference("/sdcard/kantv/mnist-ggml-model-f32.gguf", "/sdcard/kantv/mnist-5.png",
                            0, 4, QNN_BACKEND_GGML);
            break;
        }
        case 1: {
