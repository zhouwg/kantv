...
const std::string test::build_commit = "unknown";//LLAMA_COMMIT;
const int         test::build_number = 1;//LLAMA_BUILD_NUMBER;
...
struct markdown_printer : public printer {
    ...
    void print_header(const cmd_params & params) override {
        ...
        for (const auto & field : fields) {
            ...
            tmpstring << get_field_display_name(field).c_str();
            tmpstring << " | ";
        }
        fprintf(fout, "\n");
        fprintf(fout, "|");
    ...
    void print_test(const test & t) override {
        ...
        for (const auto & field : fields) {
            ...
            tmpstring << value.c_str() << " ";
        }
        fprintf(fout, "\n");
        tmpstring << "\n";
        GGML_JNI_NOTIFY("%s", tmpstring.str().c_str());
    }
...
int  ggml_bench_llama(const char * model_path, int num_threads) {
...
#if !defined(NDEBUG)
    LOGGD("warning: asserts enabled, performance may be affected\n");
#endif

    //TODO: this is a lazy method
    int argc = 5;
    char *argv[] = {"llama-bench", "-m", const_cast<char *>(model_path), "-t",
                    const_cast<char *>(std::to_string(num_threads).c_str())};
    ...
    for (const auto & inst : params_instances) {
        ...
        if (!lmodel || !prev_inst || !inst.equal_mparams(*prev_inst)) {
            ...
            LOGGD("load mode %s\n", model_path);
            GGML_JNI_NOTIFY("llama load model from file:%s\n", model_path);
            lmodel = llama_load_model_from_file(inst.model.c_str(), inst.to_llama_mparams());
            if (lmodel == NULL) {
                LOGGD( "%s: error: failed to load model '%s'\n", __func__, inst.model.c_str());
                return 1;
            }
            LOGGD("after load mode %s\n", model_path);
            prev_inst = &inst;
        }
        ...
        if (ctx == NULL) {
            LOGGD("%s: error: failed to create context with model '%s'\n", __func__, inst.model.c_str());
            llama_free_model(lmodel);
            return 1;
        }
