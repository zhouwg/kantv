...
struct SDParams {
    ...
    std::string output_path = "/sdcard/output.png";
...
int sd_inference_main(int argc, const char* argv[], int backend_type) {
    SDParams params;
    ...
    for (int i = 0; i < params.batch_count; i++) {
        ...
        LOGGD("final_image_path %s", final_image_path.c_str());
        if(is_jpg) {
            stbi_write_jpg(final_image_path.c_str(), results[i].width, results[i].height, results[i].channel,
                           results[i].data, 90, get_image_params(params, params.seed + i).c_str());
            LOGGD("save result JPEG image to '%s'\n", final_image_path.c_str());
        } else {
            stbi_write_png(final_image_path.c_str(), results[i].width, results[i].height, results[i].channel,
                           results[i].data, 0, get_image_params(params, params.seed + i).c_str());
            LOGGD("save result PNG image to '%s'\n", final_image_path.c_str());
        }

        GGML_JNI_NOTIFY("text2image-timings final_image_path %s", final_image_path.c_str());
