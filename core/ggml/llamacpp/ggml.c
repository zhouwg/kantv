...
#ifdef GGML_USE_QNN
#include "ggml-qnn.h"
static void ggml_compute_forward_mul_mat_for_qnn(const struct ggml_compute_params * params,
...
struct ggml_context {
    ...
    bool   use_hwaccel;   // for PoC: Add Qualcomm mobile SoC native backend for GGML,https://github.com/zhouwg/kant      v/issues/121
...
void ggml_compute_forward(struct ggml_compute_params * params, struct ggml_tensor * tensor, struct ggml_compute_state * state) {
    ...
    switch (tensor->op) {
        ...
        case GGML_OP_MUL_MAT:
            {
#ifdef GGML_USE_QNN
                ggml_compute_forward_mul_mat_for_qnn(params, tensor);
#else
...
struct ggml_tensor * ggml_concat_org(
    struct ggml_context * ctx,
    struct ggml_tensor * a,
    struct ggml_tensor * b) {
...
//05-26-2024, this function is used for avoid crash on Xiaomi14 because implementation of ggml_compute_forward has changed
//from
//
...
//
//
//but I can't construct the third param which is the rootcause of crash on Xiaomi14
static void ggml_compute_forward_mul_mat_for_qnn(
        const struct ggml_compute_params * params,
              struct ggml_tensor * dst) {
