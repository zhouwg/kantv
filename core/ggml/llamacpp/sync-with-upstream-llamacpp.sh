...
LLAMACPP_SRCS=(ggml-alloc.c ggml-alloc.h ggml-backend.c ggml-backend.h ggml-backend-impl.h ggml.c ggml.h ggml-quants.c ggml-quants.h llama.cpp llama.h unicode.h unicode.cpp unicode-data.h unicode-data.cpp ggml-common.h sgemm.h sgemm.cpp examples/main/main.cpp ggml-vulkan.h ggml-vulkan.cpp ggml-vulkan-shaders.hpp)
for file in "${LLAMACPP_SRCS[@]}"; do
    /bin/cp -fv ${UPSTREAM_LLAMACPP_PATH}/$file ${LOCAL_LLAMACPP_PATH}/$file
done

/bin/cp -fv ${UPSTREAM_LLAMACPP_PATH}/common/* ${LOCAL_LLAMACPP_PATH}/common/
