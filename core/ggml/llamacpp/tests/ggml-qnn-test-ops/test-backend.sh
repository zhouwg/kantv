...
function run_ggml_qnn_ut()
{
    ...
    case "$testmode" in
        "test")
            adb shell ${REMOTE_PATH}/${GGML_QNN_UT}  test
        ;;

        "perf")
            adb shell ${REMOTE_PATH}/${GGML_QNN_UT}  perf
        ;;

        *)
            adb shell ${REMOTE_PATH}/${GGML_QNN_UT}  test -o $ggmlop -b $qnnbackend
        ;;
    esac
}
...
function show_usage()
{
    ...
    echo "  $0 test"
    echo "  $0 perf"
    echo "  $0 ADD/MUL/MUL_MAT  qnn-cpu/qnn-gpu/qnn-npu"
    echo -e "\n\n\n"
}
