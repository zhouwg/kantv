...
public:
    ...
    struct ggml_tensor* forward(struct ggml_context* ctx, struct ggml_tensor* pixel_values) {
        ...
        struct ggml_tensor* x = ggml_concat_org(ctx, class_embedding, patch_embedding);    // [N, num_positions, embed_dim, 1]
...
struct FrozenCLIPEmbedderWithCustomWords : public GGMLModule {
    ...
    struct ggml_tensor* forward(struct ggml_context* ctx,
        ...
        if (version == VERSION_XL) {
            ...
            hidden_states = ggml_concat_org(ctx, hidden_states, hidden_states2);  // [N, n_token, hidden_size + hidden_size2]
    ...
    struct ggml_cgraph* build_graph(struct ggml_tensor* input_ids,
        ...
        if (num_custom_embeddings > 0 && version != VERSION_XL) {
            ...
            embeddings = ggml_concat_org(compute_ctx, token_embed_weight, custom_embeddings);
