...
public:
    ...
    struct ggml_tensor* forward(struct ggml_context* ctx, struct ggml_tensor* x) {
        ...
        // upsample
        feat     = lrelu(ctx, conv_up1->forward(ctx, ggml_upscale(ctx, feat, 2, GGML_SCALE_MODE_BILINEAR)));
        feat     = lrelu(ctx, conv_up2->forward(ctx, ggml_upscale(ctx, feat, 2, GGML_SCALE_MODE_BILINEAR)));
