...
public:
    ...
    struct ggml_tensor* fuse_fn(struct ggml_context* ctx,
        ...
        auto stacked_id_embeds = ggml_concat_org(ctx, prompt_embeds0, id_embeds0);
    ...
    struct ggml_tensor* forward(struct ggml_context* ctx,
        ...
        if (left && right) {
            stacked_id_embeds = ggml_concat_org(ctx, left, stacked_id_embeds);
            stacked_id_embeds = ggml_concat_org(ctx, stacked_id_embeds, right);
        } else if (left) {
            stacked_id_embeds = ggml_concat_org(ctx, left, stacked_id_embeds);
        } else if (right) {
            stacked_id_embeds = ggml_concat_org(ctx, stacked_id_embeds, right);
...
struct PhotoMakerIDEncoderBlock : public CLIPVisionModelProjection {
    ...
    struct ggml_tensor* forward(struct ggml_context* ctx,
        ...
        id_embeds = ggml_concat_org(ctx, id_embeds, id_embeds_2);  // [batch_size, seq_length, 1, 2048] check whether concat at dim 2 is right
