                         ...
                         int *got_packet, const AVFrame *frame)
{
    ...
    *got_packet = 0;
    ...
    //2023-12-24,weiguo, added for troubleshooting kantv-record
    if (NULL == avctx) {
        LOGJ("codec context is NULL\n");
        return -1;
    }
    if (NULL == avctx->codec) {
        LOGJ("codec context->codec is NULL\n");
        return -1;
    }
    //end added
    ...
    av_log(avctx, AV_LOG_INFO, "codec name: %s(%s,%d)\n", avctx->codec->name, avctx->codec->long_name, avctx->codec->id);
    av_log(avctx, AV_LOG_INFO, "codec cap: 0x%x\n", avctx->codec->capabilities);
    if (avctx->codec->capabilities & AV_CODEC_CAP_DR1) {
#if 0//weiguo,2023-12-19
