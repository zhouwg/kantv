...
function build_x86
{
cmake -H. -B./out/x86 -DPRJ_ROOT_PATH=${PROJECT_ROOT_PATH} -DTARGET_NAME=${TARGET} -DCMAKE_BUILD_TYPE=${LIB_BUILD_TYPE} -DBUILD_LIB_TYPE=${LIB_TYPE} -DPREFIX=${FF_PREFIX} -DCMAKE_INSTALL_PREFIX=${FF_PREFIX} -DCMAKE_POSITION_INDEPENDENT_CODE=ON -DCMAKE_VERBOSE_MAKEFILE=ON  -DBUILD_SHARED_LIBS=false
cd ./out/x86
make -j${HOST_CPU_COUNTS} VERBOSE=1
make install
cd -
