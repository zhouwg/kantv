...
{
LIB_TYPE=SHARED
cmake -H. -B./out/x86 -DPRJ_ROOT_PATH=${PROJECT_ROOT_PATH} -DTARGET_NAME=${TARGET} -DCMAKE_BUILD_TYPE=${LIB_BUILD_TYPE} -DBUILD_LIB_TYPE=${LIB_TYPE} -DPREFIX=${FF_PREFIX} -DCMAKE_INSTALL_PREFIX=${FF_PREFIX} -DCMAKE_POSITION_INDEPENDENT_CODE=ON
cd ./out/x86
make -j${HOST_CPU_COUNTS}
make install
cd -
