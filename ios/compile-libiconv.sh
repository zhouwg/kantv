...
do_lipo () {
    ...
    do
        LIPO_FLAGS="$LIPO_FLAGS $UNI_BUILD_ROOT/build/libiconv-$ARCH/output/lib/$LIB_FILE"
    done
...
do_lipo_all () {
    ...
    cp -R $UNI_BUILD_ROOT/build/libiconv-x86_64/output/include $UNI_BUILD_ROOT/build/universal/
}

#----------
if [ "$FF_TARGET" = "armv7" -o "$FF_TARGET" = "armv7s" -o "$FF_TARGET" = "arm64" ]; then
    echo_archs
    sh tools/do-compile-libiconv.sh $FF_TARGET
    do_lipo_all
    ...
    for ARCH in $FF_ALL_ARCHS
    do
        sh tools/do-compile-libiconv.sh $ARCH
    done
    ...
    for ARCH in $FF_ALL_ARCHS
    do
        cd libiconv-$ARCH && git clean -xdf && cd -
    done
else
    echo "Usage:"
    echo "  compile-libiconv.sh armv7|arm64|i386|x86_64"
    echo "  compile-libiconv.sh lipo"
    echo "  compile-libiconv.sh all"
    echo "  compile-libiconv.sh clean"
    echo "  compile-libiconv.sh check"
    exit 1
fi
