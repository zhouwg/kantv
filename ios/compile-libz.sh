...
do_lipo () {
    ...
    do
        LIPO_FLAGS="$LIPO_FLAGS $UNI_BUILD_ROOT/build/libz-$ARCH/output/lib/$LIB_FILE"
    done
...
do_lipo_all () {
    ...
    cp -R $UNI_BUILD_ROOT/build/libz-x86_64/output/include $UNI_BUILD_ROOT/build/universal/
}
...
if [ "$FF_TARGET" = "armv7" -o "$FF_TARGET" = "armv7s" -o "$FF_TARGET" = "arm64" ]; then
    ...
    for ARCH in $FF_ALL_ARCHS
    do
        sh tools/do-compile-libz.sh $ARCH
    done
    ...
    for ARCH in $FF_ALL_ARCHS
    do
        cd libz-$ARCH && git clean -xdf && cd -
    done
else
    echo "Usage:"
    echo "  compile-libz.sh armv7|arm64|i386|x86_64"
    echo "  compile-libz.sh lipo"
    echo "  compile-libz.sh all"
    echo "  compile-libz.sh clean"
    echo "  compile-libz.sh check"
    exit 1
fi
