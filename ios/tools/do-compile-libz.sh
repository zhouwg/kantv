...
# i386, x86_64
LIBZ_CFG_FLAGS_SIMULATOR=

# armv7, armv7s, arm64
LIBZ_CFG_FLAGS_ARM=
LIBZ_CFG_FLAGS_ARM="iphoneos-cross"
...
if [ "$FF_ARCH" = "i386" ]; then
    FF_BUILD_NAME="libz-i386"
    FF_XCRUN_PLATFORM="iPhoneSimulator"
    FF_XCRUN_OSVERSION="-mios-simulator-version-min=6.0"
    LIBZ_CFG_FLAGS="darwin-i386-cc $LIBZ_CFG_FLAGS"
elif [ "$FF_ARCH" = "x86_64" ]; then
    FF_BUILD_NAME="libz-x86_64"
    FF_XCRUN_PLATFORM="iPhoneSimulator"
    FF_XCRUN_OSVERSION="-mios-simulator-version-min=7.0"
    LIBZ_CFG_FLAGS="darwin64-x86_64-cc $LIBZ_CFG_FLAGS"
elif [ "$FF_ARCH" = "armv7" ]; then
    FF_BUILD_NAME="libz-armv7"
    FF_XCRUN_OSVERSION="-miphoneos-version-min=6.0"
    FF_XCODE_BITCODE="-fembed-bitcode"
    LIBZ_CFG_FLAGS="$LIBZ_CFG_FLAGS_ARM $LIBZ_CFG_FLAGS"
#    LIBZ_CFG_CPU="--cpu=cortex-a8"
elif [ "$FF_ARCH" = "armv7s" ]; then
    FF_BUILD_NAME="libz-armv7s"
    LIBZ_CFG_CPU="--cpu=swift"
    FF_XCRUN_OSVERSION="-miphoneos-version-min=6.0"
    FF_XCODE_BITCODE="-fembed-bitcode"
    LIBZ_CFG_FLAGS="$LIBZ_CFG_FLAGS_ARM $LIBZ_CFG_FLAGS"
elif [ "$FF_ARCH" = "arm64" ]; then
    FF_BUILD_NAME="libz-arm64"
...
#--------------------
echo "\n--------------------"
echo "[*] configurate libz"
echo "--------------------"

LIBZ_CFG_FLAGS="--prefix=$FF_BUILD_PREFIX --static"
LIBZ_CFG_FLAGS="$LIBZ_CFG_FLAGS"
...
echo "./configure $LIBZ_CFG_FLAGS"
./configure $LIBZ_CFG_FLAGS
make clean
